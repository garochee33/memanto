...
class TestBasicMapping:
    ...
    def test_raw_list_export_shape(self):
        """Real ChatGPT conversations.json is a top-level array (not a dict)."""
        conv = _make_conversation("Test", "conv-list", [
            ("m1", "user", "What is Python?", 1700000100.0),
            ("m2", "assistant", "A programming language.", 1700000105.0),
        ])
        rows = map_chatgpt([conv])
        assert len(rows) == 1
        assert rows[0]["source"] == "chatgpt"
        assert "What is Python?" in rows[0]["content"]
