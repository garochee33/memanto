...
def _conversations_from_export(export: dict[str, Any] | list[Any]) -> list[Any]:
    """Normalize ChatGPT export shapes to a conversation list.

    Real OpenAI exports are a top-level JSON **array**. Callers (and older
    fixtures) may also pass ``{"conversations": [...]}``. Accept both so
    ``map_chatgpt`` never does ``list.get`` (AttributeError on real exports).
    """
    if isinstance(export, list):
        return export
    if not isinstance(export, dict):
        raise TypeError(
            f"ChatGPT export must be a list or dict, got {type(export).__name__}"
        )
    if "conversations" in export:
        conversations = export.get("conversations") or []
    elif "mapping" in export:
        # Single conversation object
        conversations = [export]
    else:
        conversations = export.get("memories") or []
    if not isinstance(conversations, list):
        raise TypeError(
            "ChatGPT 'conversations' must be a list, "
            f"got {type(conversations).__name__}"
        )
    return conversations


def map_chatgpt(export: dict[str, Any] | list[Any]) -> list[dict[str, Any]]:
    ...
    Accepts either:
    - a raw ChatGPT ``conversations.json`` array (real export), or
    - ``{"conversations": [...]}`` (loader / test fixture wrapper)
    ...
    conversations = _conversations_from_export(export)
